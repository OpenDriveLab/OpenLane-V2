...
def pairwise(xs: list, ys: list, distance_function: callable, mask: np.ndarray = None) -> np.ndarray:
    r"""
    ...
    for i, x in enumerate(xs):
        for j, y in enumerate(ys):
            if mask is None or mask[i][j]:
                result[i][j] = distance_function(x, y)
    return result
...
def chamfer_distance(gt: np.ndarray, pred: np.ndarray) -> float:
    ...
    Notes
    -----
    Adapted from https://github.com/Mrmoore98/VectorMapNet_code/blob/810ae463377f8e724c90a732361a675bcd7cf53b/plugin/models/assigner/match_cost.py#L12.
