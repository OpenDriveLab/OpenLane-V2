...
def collect_lanesegment(root_path : str, data_dict : dict, collection : str, n_points : int = 10, with_sd_map : bool = False) -> None:
    r"""
    Load meta data of data in data_dict,
    and store in a .pkl with split as file name.
...
def fix_pts_interpolate(lane, n_points):
    ...
    lane = np.array([ls.interpolate(distance).coords[0] for distance in distances], dtype=np.float32)
    return lane
